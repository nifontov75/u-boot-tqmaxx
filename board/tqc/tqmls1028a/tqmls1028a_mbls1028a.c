...
static uint16_t _rgmii_phy_read_indirect(struct mii_dev *ext_bus, uint8_t addr)
{
	...
	ext_bus->write(ext_bus, RGMII_PHY_DEV_ADDR, MDIO_DEVAD_NONE, 0x0e, addr);
	ext_bus->write(ext_bus, RGMII_PHY_DEV_ADDR, MDIO_DEVAD_NONE, 0x0d, 0x401f);
	return ext_bus->read(ext_bus, RGMII_PHY_DEV_ADDR, MDIO_DEVAD_NONE, 0x0e);
}
